...
if (isset($_POST["activate_boost"])) {
    if ($expiry == 0) {
        if ($kingdom->get_kingdom_food() >= $boost_cost) {
            $kingdom->give_kingdom_food(-$boost_cost);

            $res_ft = $db_instance->execute_query("SELECT ft.foodrate FROM map m JOIN fieldtypes ft ON m.fieldtype = ft.fieldid WHERE m.kingdomid = ?", [$current_kingdom]);
...
if ($expiry > 0) {
    ...
    $view .= "<form method='POST'>
                <button type='submit' name='activate_boost'>
                    Boost für " . get_resource_icon(ResourceTypes::RESOURCE_TYPE_FOOD) . " " . fnum($boost_cost) . "
                </button>
            </form>";
}
