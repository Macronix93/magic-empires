...
$result = check_user_login_and_kingdom($user, BuildingTypes::BUILDING_SAWMILL);
...
$res_type = ResourceTypes::RESOURCE_TYPE_WOOD;
...
if (isset($_POST["activate_boost"])) {
    if ($ticks_left == 0) {
        if ($user->get_user_coins() >= $boost_cost) {
            ...
            $res_ft = $db_instance->execute_query("SELECT ft.woodrate FROM map m JOIN field_types ft ON m.fieldtype = ft.fieldid WHERE m.kingdomid = ?", [$current_kingdom]);
...
$boost_value = (int)($ft_res["woodrate"] * BASE_WOOD_GAIN);


$next_level_text = "";
if ($lvl < MAX_BUILDING_LEVEL) {
    $next_increase = (int)round(BASE_WOOD_GAIN * $ft_res["woodrate"]);
    $next_level_text = "<div class='split-content'><b>Nächste Stufe:</b><div class='passed'>" . fnum($kingdom->get_base_wood_rate() + $next_increase) . " (+" . fnum($next_increase) . ")</div></div>";
}

$view .= "<div style='margin: auto; width: 350px;'>
                <div class='split-content'><b>Holzertrag pro Stunde:</b> " . fnum($kingdom->get_base_wood_rate()) . " $boost_display</div>
                $next_level_text
            </div>
";
