...
if (isset($_POST["activate_boost"])) {
    if ($expiry == 0) {
        if ($kingdom->get_kingdom_wood() >= $boost_cost) {
            $kingdom->give_kingdom_wood(-$boost_cost);

            $res_ft = $db_instance->execute_query("SELECT ft.woodrate FROM map m JOIN fieldtypes ft ON m.fieldtype = ft.fieldid WHERE m.kingdomid = ?", [$current_kingdom]);
 ...
 * HTML Content Part
 */
$view .= "<div style='margin-bottom: 15px;'><b>Holzertrag pro Stunde:</b> " . fnum($kingdom->get_kingdom_wood_per_hour()) . " $boost_display</div>";
...
if ($expiry > 0) {
    ...
    $view .= "<form method='POST'>
                <button type='submit' name='activate_boost'>
                    Boost für " . get_resource_icon(ResourceTypes::RESOURCE_TYPE_WOOD) . " " . fnum($boost_cost) . "
                </button>
            </form>";
}
