                        ...
                        if (isset($_POST["sendpm"])) {
                            ...
                            if ($error !== null) {
                                ...
                                if ($sum >= MAX_USER_MESSAGES) {
                                    echo "Der Posteingang dieses Benutzers ist voll!";
                                } else {
                                    $stmt = $mysqli->prepare("SELECT COUNT(*) FROM users WHERE username = ?");
                                    $stmt->bind_param('s', $receiver); // Assuming $username is the username you want to check
                                    $stmt->execute();
                            ...
                            if (isset($_GET["action"])) {
                                if ($_GET["action"] == "folder") {
                                        ...
                                        if ($check != 0) {
                                            ...
                                            <table class=table>
                                                <tr>
                                                    ...
                                                    <td class="td-center td-gradient"><b>Absender</b></td>
                                                    <td class="td-center td-gradient"><b>Datum & Zeit</b></td>
                                                    <td class="td-center td-gradient"><b>Aktion</b></td>
                                                </tr>
                                                <?php
                                                while ($row = $query->fetch_assoc()) {
                                                    echo "<tr><td><a href='messages.php?action=read&m_id=" . $row["id"] . "'>" . $row["subject"] . "</a> " . ($row["hasread"] ? "" : "<b>(neu!)</b>") . "</td><td>" . $row["sender"] . "</td><td>" . date("d.m.Y H:i:s", $row["date"]) . "</td>
                                                                <td style='text-align: center;'><a href='messages.php?action=read&m_id=" . $row["id"] . "'><img src='images/icons/icon_loupe.png' class='menu-icons' alt='Lesen'></a><a href='messages.php?action=delete&m_id=" . $row["id"] . "'><img src='images/icons/icon_delete.png' class='menu-icons' alt='Löschen'></a></td></tr>";
                                                }
                                                ?>
                                            </table>
                                            <?php
                                    ...
                                    ?>
                                    <form name="newmessage" action="messages.php" method="post" style="width: 100%;">
                                        <table class="table">
                                            <tr>
                                                <td style="width: 25%;">
                                                    <b>Empfänger:</b>
