...
$result = check_user_login_and_kingdom($user, $db_instance, BuildingTypes::BUILDING_GOLDMINE);
...
if (isset($_POST["activate_boost"])) {
    if ($expiry == 0) {
        if ($kingdom->get_kingdom_gold() >= $boost_cost) {
            $kingdom->give_kingdom_gold(-$boost_cost);

            $res_ft = $db_instance->execute_query("SELECT ft.goldrate FROM map m JOIN fieldtypes ft ON m.fieldtype = ft.fieldid WHERE m.kingdomid = ?", [$current_kingdom]);
 ...
 * HTML Content Part
 */
$view .= "<div style='margin-bottom: 15px;'><b>Goldertrag pro Stunde:</b> " . fnum($kingdom->get_kingdom_gold_per_hour()) . " $boost_display</div>";
