...
$result = check_user_login_and_kingdom($user, BuildingTypes::BUILDING_GOLDMINE);
...
$res_type = ResourceTypes::RESOURCE_TYPE_GOLD;
...
if (isset($_POST["activate_boost"])) {
    if ($ticks_left == 0) {
        if ($user->get_user_coins() >= $boost_cost) {
            ...
            $res_ft = $db_instance->execute_query("SELECT ft.goldrate FROM map m JOIN field_types ft ON m.fieldtype = ft.fieldid WHERE m.kingdomid = ?", [$current_kingdom]);
            $ft = $res_ft->fetch_assoc();

            $hourly_boost = (int)round(BASE_GOLD_GAIN * $ft["goldrate"] * BOOST_PRODUCTION_BONUS);
...
if ($lvl < MAX_BUILDING_LEVEL) {
    $next_increase = (int)round(BASE_GOLD_GAIN * $ft_res["goldrate"]);
    $next_level_text = "<div class='split-content'><b>Nächste Stufe:</b><div class='passed'>" . fnum($kingdom->get_base_gold_rate() + $next_increase) . " (+" . fnum($next_increase) . ")</div></div>";
}

$view .= "<div style='margin: auto; width: 350px;'>
                <div class='split-content'><b>Goldertrag pro Stunde:</b> " . fnum($kingdom->get_base_gold_rate()) . " $boost_display</div>
                $next_level_text
            </div>
";
